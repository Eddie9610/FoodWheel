...
async function loadConfig({ configPath, configFileName, requireConfig = false, name, type }) {
    ...
    envFileNames.forEach(envFile => {
        ...
        if (fs_1.existsSync(dotEnvPath) && fs_1.lstatSync(dotEnvPath).isFile()) {
            ...
            if (legacyKey && key) {
                throw new Error(`Cannot set both ${exports.legacyKeyEnvVar} and ${exports.keyEnvVar}. Please only set ${exports.keyEnvVar}`);
            }
            if (legacyKey) {
                utilities_1.Debug.warning(`[Deprecation warning] Setting the key via ${exports.legacyKeyEnvVar} is deprecated and will not be supported in future versions.`);
            }
